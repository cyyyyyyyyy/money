...
from money_strategy.validation import build_validation_report, validation_summary_frame
...
def test_validation_report_flags_short_shifted_history() -> None:
    index = pd.date_range("2024-01-05", periods=8, freq="W-FRI")
    weekly_close = pd.DataFrame(
        {
            "000300": [100, 101, 102, 101, 103, 104, 103, 105],
    ...
    signals = pd.DataFrame(
        {
            "bucket": ["balanced"] * len(index),
            "weight_159819": [1.0] * len(index),
            "weight_512480": [0.0] * len(index),
            "weight_159770": [0.0] * len(index),
            "weight_516160": [0.0] * len(index),
            "weight_561560": [0.0] * len(index),
            "weight_515100": [0.0] * len(index),
            "weight_512800": [0.0] * len(index),
        },
        index=index,
    )
    result = run_backtest(weekly_close, signals)
    ...
    report = build_validation_report(
        ...
        requested_end="2024-02-23",
        cost_bps=10.0,
        sentiment_file=None,
        params={"switch_threshold": 0.15},
    )
    summary = validation_summary_frame(report)

    assert report["method"] == "rule_based_no_training"
    assert report["effective_period"]["start"] == "2024-01-05"
    assert any(warning.startswith("effective_history_short") for warning in report["warnings"])
    assert any(warning.startswith("start_shifted_by_data_availability") for warning in report["warnings"])
    assert summary.loc[0, "period"] == "full"
